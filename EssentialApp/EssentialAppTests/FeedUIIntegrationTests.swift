...
//
//  Created by PT.Koanba on 20/02/22.
//
...
class FeedUIIntegrationTests: XCTestCase {
    ...
    func test_imagesSelection_notifiesHandler() {
    ...
    func test_loadingMoreIndicator_isVisibleWhileLoadingMore() {
        ...
        XCTAssertFalse(sut.isShowingLoadMoreFeedIndicator, "Expected no loading indicator once user is initiated loading completes successfully")
        
        sut.simulateLoadMoreFeedAction()
        XCTAssertTrue(sut.isShowingLoadMoreFeedIndicator, "Expected loading indicator on load more action")
        
        loader.completeLoadMoreWithError(at: 1)
        XCTAssertFalse(sut.isShowingLoadMoreFeedIndicator, "Expected no loading indicator once user is initiated loading completes with error")
    ...
    func test_tapOnLoadMoreErrorView_loadsMore() {
        ...
        sut.simulateTapLoadMoreFeedError()
        XCTAssertEqual(loader.loadMoreCallCount, 1)
    ...
    func test_feedImageView_loadsImageURLWhenVisible() {
        ...
        XCTAssertEqual(loader.loadedImageURLs, [image0.url, image1.url], "Expected second image URL request once first view also becomes visible")
    }
    
    func test_feedImageView_cancelsImageLoadingLWhenNotVisibleAnymore() {
        let image0 = makeImage(url: URL(string: "http://url-0.com")!)
        let image1 = makeImage(url: URL(string: "http://url-1.com")!)
        let (sut, loader) = makeSUT()
        ...
        sut.loadViewIfNeeded()
        loader.completeFeedLoading(with: [image0, image1])
        XCTAssertEqual(loader.cancelledImageURLs, [], "Expected no image URL requests until image is not visible")
    ...
    func test_feedImageViewLoadingIndicator_isVisibleWhileLoadingImage() {
        ...
        XCTAssertEqual(view1?.isShowingImageLoadingIndicator, false, "Expected no loading for second view once second image loading completes with error")
    }
    ...
    func test_feedImageView_rendersImageLoadedFromURL() {
        ...
        XCTAssertEqual(view1?.renderedImage, .none, "Expected no image state change for second view once image loading completes successfully")
        
        let imageData1 = UIImage.make(withColor: .blue).pngData()!
        loader.completeImageLoading(with: imageData1, at: 1)
        XCTAssertEqual(view0?.renderedImage, imageData0, "Expected no image state chage for first view once second image loading completes successfully")
    ...
    func test_feedImageViewRetryButton_isVisibleOnImageURLLoadError() {
        ...
        XCTAssertEqual(view1?.isShowingRetryAction, false, "Expected no retry action state change for second view once image loading completes successfully")
        
        loader.completeImageLoadingWithError(at: 1)
        XCTAssertEqual(view0?.isShowingRetryAction, false, "Expected no retry action state chage for first view once second image loading completes error")
