...
//
//  Created by PT.Koanba on 15/02/22.
//
...
class EssentialFeedCacheIntegrationTests: XCTestCase {
    ...
    func test_load_deliversItemsSavedOnASeparateInstance() {
        let feedLoaderPerformSave = makeFeedLoader()
        let feedLoaderToPerformLoad = makeFeedLoader()
        let feed = uniqueImageFeed().models
        
        save(feed, with: feedLoaderPerformSave)
    ...
    func test_saveImageData_overridesSaveImageDataOnASeparateInstance() {
    ...
    private func testSpecificStoreURL() -> URL {
        return cachesDiretory().appendingPathComponent("\(type(of: self)).store")
    }
    
    private func cachesDiretory() -> URL {
